...
def next_block(last_block):
    ...
    return Block(i, datetime.now(), "this is block " + str(i), last_block.hash)
    
# append 5 blocks to the blockchain
def app_five(block_list):
    for i in range(len(block_list), len(block_list) + 5):
        block_list.append(next_block(block_list[i-1]))